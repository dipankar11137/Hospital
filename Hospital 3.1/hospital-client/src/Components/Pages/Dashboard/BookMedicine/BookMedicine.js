...
const BookMedicine = () => {
  ...
  const fetchBookings = async () => {
    try {
      const response = await fetch('http://localhost:5000/medicineBookings');
  ...
  const handleDelete = async id => {
    if (window.confirm('Are you sure you want to delete this booking?')) {
      try {
        const res = await fetch(`http://localhost:5000/medicineBooking/${id}`, {
          method: 'DELETE',
        });
              ...
              {bookings
                ...
                .map((med, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">{med.medicineName}</td>
                    <td className="px-4 py-3">{med.medicineGroup}</td>
                    <td className="px-4 py-3">{med.medicineType}</td>
                    <td className="px-4 py-3 font-semibold">{med.quantity}</td>
                    <td className="px-4 py-3 font-bold text-green-600">
                      ৳ {med.totalPrice}
                    </td>
                    <td className="px-4 py-3">{med.location}</td>
                    <td className="px-4 py-3">{med.name}</td>
                    <td className="px-4 py-3">{med.phone}</td>
